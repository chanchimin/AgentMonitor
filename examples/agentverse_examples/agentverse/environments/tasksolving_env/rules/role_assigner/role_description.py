    ...
    async def astep(
        ...
        **kwargs,
        ...
        if len(roles.content) == 0:
            roles.content = ["a mathematician specializing in number theory", "a statistician with expertise in probability theory"]
        if len(roles.content) < len(group_members):
            roles.content = [
                 "a mathematician specializing in number theory",
                 "a computer scientist with expertise in algorithms",
                 "a data scientist proficient in handling numerical data",
                 "a software engineer experienced in Python programming"
            ]
        if len(roles.content) < len(group_members):
            raise ValueError(
