...
async def do_monitor(agent, monitor):
    await monitor.register(agent, agent.update_messages, agent.step, name=agent.role_name)
...
class RolePlaying:
    ...
    def __init__(
        ...
        if self.monitor == None:
            raise ValueError("monitor not found (3).")
        ...
        if with_task_specify:
            ...
            asyncio.run(do_monitor(task_specify_agent, self.monitor))
        ...
        if with_task_planner:
            ...
            asyncio.run(do_monitor(task_planner_agent, self.monitor))
        ...
        asyncio.run(do_monitor(self.assistant_agent, self.monitor))
        self.user_agent: ChatAgent = ChatAgent(self.user_sys_msg,memory, model_type, **(user_agent_kwargs or {}), )
        asyncio.run(do_monitor(self.user_agent, self.monitor))
